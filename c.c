...
{		
	...
	for(n;;n++)
	{
		...
		pnext=(zg *)malloc(sizeof(zg));
		fread(pnext,sizeof(struct zg),1,fp);  //每次从文件中读取一条数据到结构体
//		grsds();
		plast->next=pnext;
		pnext->previous=plast;
		plast=plast->next;	
...
/*int write()                                   //5.保存模块
{
	...
	for(gz=header->next;;gz=gz->next)
	{
		fwrite(&gz,sizeof(struct zg),1,fp);	//把结构体中的数据写入文件
		if(gz->next==NULL)                       //判断文件是否到末尾
		{
			break;
		}
	}
...
int list()                        //显示模块
{
	...
	for(i=0;i<n;i++)
	{
		...
		printf("%-10s",tem->next->gh);
		printf("%-10s",tem->next->name);
		printf("%12.2f\t",tem->next->gwgz);
		printf("%8.2f\t",tem->next->xjgz);
		printf("%8.2f\t",tem->next->zwjt);
		printf("%8.2f\t",tem->next->jxgz);
		printf("%8.2f\t",tem->next->yfgz);
		printf("%8.2f\t",tem->next->grsds);
		printf("%8.2f\t\n",tem->next->sfgz);
		tem=tem->next;
	}
	system("pause");
